...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Get current member data from page
    ...
    function showMemberDetails(member) {
        // Set modal title
        document.getElementById('member-modal-title').textContent = `${member.username}'s Details`;
        ...
        memberDetails.innerHTML = `
            ...
            <div class="member-detail-row">
                ...
                <span class="member-detail-value">${member.isAdmin ? 'Admin' : 'Member'}</span>
            </div>
        `;
        ...
        if (isCurrentUserAdmin && member.id !== currentMemberId) {
            memberModalActions.innerHTML = `
                ...
                `}
                <button class="btn btn-danger" onclick="removeMember('${member.id}')" type="button">Remove from Group</button>
            `;
        } else {
            memberModalActions.innerHTML = `
                <button class="btn btn-secondary close-member-modal-btn" type="button">Close</button>
    ...
    window.promoteMember = async (memberId) => {
        try {
            const response = await fetch(`/api/groups/promote-admin/${memberId}`, {
                method: 'PUT',
    ...
    window.demoteMember = async (memberId) => {
        try {
            const response = await fetch(`/api/groups/demote-admin/${memberId}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' }
            });
    ...
    window.removeMember = async (memberId) => {
        ...
        try {
            const response = await fetch(`/api/groups/remove-member/${memberId}`, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' }
            });
